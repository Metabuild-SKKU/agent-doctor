...
> 문제고 "동작 안 함"은 아니다. `retrieval_temp.py`(STEP2)만 Index Agent 검색 리트리버가
> 나오면 통째로 삭제될 **의도적 임시 파일**이다.
...
세 함수로 provider 차이를 감추고, `probe_gen.py`/`retrieval_temp.py`/`metrics_ragas.py`가 전부
이 계층만 호출한다(직접 `from openai import OpenAI` 하지 않음).
...
├── llm_provider.py    # LLM 호출 추상화(OpenAI/Gemini/GitHub Models) — probe_gen/retrieval_temp/metrics_ragas 공용
...
4. **LLM 생성** (`retrieval_temp.py`) — `_llm_generate`가 `llm_provider.generate_text`를 호출.
   실제 RAG 생성기(프롬프트 엔지니어링·컨텍스트 랭킹 등)로 고도화하는 건 Index 검색 개발
   이후에도 유지될 부분.
...
6. **STEP4 라벨 세트 확장** (`diagnose.py`) — `chunking_context_mismatch`는 구현됨.
   ...
   `chunking_underchunking`/`chunking_overchunking`/`reranker_*`는 Index Agent 쪽
   세부 신호가 선행돼야 판별 가능.
7. **임시 검색 제거** — Index Agent가 검색 리트리버(`build_retriever`/`search`)를 제공하면
   `retrieval_temp.py` 를 통째로 삭제하고 `agent.py` 의 검색 호출을 Index 쪽으로 교체.
   생성 함수(`generate_answer`)만 eval 로 이관해 유지.
