...
| 설정 | 값 | 무엇 |
|------|----|------|
| Ingest | `source_type="korquad"`, `source_url=data/corpus.jsonl` | 청크를 원문으로 복원해 Document 로 수집(→ Index 가 재청킹) |
| Eval | `EVAL_PROBE_SOURCE=taxonomy` | `data/qa_pairs.jsonl` 의 질문·정답·`positive_chunk_ids` 를 Probe 로 로드 |

```bash
python run_local_pipeline.py                        # 기본이 korquad (소규모 스모크)
PIPELINE_DATASET=file python run_local_pipeline.py  # 기존 hr_policy 데모로 전환
EVAL_MODE=deep EVAL_ENABLE_LLM=1 python run_local_pipeline.py  # 생성·RAGAS 채점(API 비용)
```

동작 원리 — 어댑터(`agents/eval/datasets/korquad.py`)가 corpus 를 **원문 좌표**로 복원하고,
qa 의 `positive_chunk_ids` 를 그 좌표의 `gold_spans` 로 실어 준다. Index 가 자기 전략으로
**재청킹**한 뒤 `_resync_gold_chunk_ids` 가 gold_spans 와 겹치는 현재 청크를 다시 찾아
`gold_chunk_ids` 를 확정한다 → **chunk_size/전략이 바뀌어도(Optimize) gold 가 유지**된다.

| 환경변수 | 기본 | 설명 |
|----------|------|------|
| `PIPELINE_DATASET` | `korquad` | `run_local_pipeline.py` 데이터셋 (`korquad`/`file`) |
| `KORQUAD_MAX_DOCS` | (스모크 20) | 앞 N개 문서만. 0/미설정=전체. corpus·qa 동일 규칙 |
| `KORQUAD_QA_LIMIT` | (스모크 50) | qa 개수 상한. 0/미설정=전체 |
| `EVAL_TAXONOMY_QA` / `EVAL_TAXONOMY_CORPUS` | `data/qa_pairs.jsonl` / `data/corpus.jsonl` | taxonomy 파일 경로 |
