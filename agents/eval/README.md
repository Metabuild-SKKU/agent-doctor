...
실측(`output/logs/corpus_20260804_103059.txt`) 30문항 중 **14건(47%)** 이 `recall=0` 이었고,
다수가 골드 청크 2~4개를 **전부** top-k 에 넣어야만 1점이 되는 구조적 불가능이었다.

이제 **덮은 문자 수 / 골드 전체 문자 수**(span 이 여럿이면 그 평균)를 쓴다. LLM 은 쓰지
않는다 — 좌표 산수라 결정론적이고 비용이 0이다.
