...
> **갱신(#141, 2026-08-14): 마진 도달 가능성이 tier보다 앞선다.**
>
> 위 불변조건은 "상류를 고치면 하류 측정이 흔들린다"는 전제 위에 선다. A 후보가
> 겨냥한 probe 가 극소수면 그 전제가 성립하지 않는다 — 처방이 완벽히 성공해도
> 종합점수를 개선 마진만큼 못 올려 KEEP 판정 자체가 불가능하고, 그동안 훨씬 큰
> 하위 tier 후보가 계속 밀린다. 실측(`pipeline_20260814_105310`·`_133343`)에서
> probe 1개짜리 A 후보가 probe 14개짜리 B 후보를 두 실행 모두 눌렀고 롤백됐다.
>
> 그래서 정렬 키 0번으로 **마진 도달 가능성**을 둔다
> (`action_aggregator.annotate_margin_reachability`). 겨냥한 probe 가 전부 만점이
> 됐을 때의 종합점수 상승폭을 `MIN_IMPROVEMENT_MARGIN` 과 비교한다.
>
> **이건 group multiplier가 아니다.** multiplier를 거부한 이유는 지지가 몰리면
> 역전을 허용하기 때문인데(`10×1 > 2×3`), 여기서는 마진에 닿는 A 후보가 하위 tier의
> 지지가 아무리 많아도 우선권을 유지한다. 점수로 역전되지 않는다. 우선권을 잃는 것은
> 물리적으로 KEEP 판정을 통과할 수 없는 후보뿐이며, 지지 수 경쟁이 아니라 도달
> 가능성의 이진 게이트다. 둘 다 미달이면 같은 층이라 tier 순서가 그대로 남는다.
>
> 탈락이 아니라 **강등**이다 — 마진에 닿는 대안이 없으면 그 후보가 그대로 선택된다.
> 성분 미측정·probe 총수 불명이면 판단을 보류하고 기존 순서를 지킨다.
>
> 남은 것: **정책 객체 분리(위 세 번째 항목)는 아직 하지 않았다.** #141은 분리 없이
> 기본 동작을 바꿨고, 근거는 위 실측이다. 분리 계층이 필요해지면 그 위에 얹는다.
...
| A>C>B hard tier | **불변조건**, multiplier 미채택. 단 마진에 못 닿는 후보는 우선권 강등(#141) | §4.3 |
