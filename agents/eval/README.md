...
/ `graph.py`)이 설정만으로 돈다. 소스는 `SOURCE_TYPE`/`SOURCE_URL` env 하나로 넣고(=state),
`korquad` 면 Eval 이 자동으로 taxonomy 소스를 쓴다:
...
> corpus 경로는 **한 곳(`SOURCE_URL`=state.source_url)** 에서만 정한다. Eval 은 gold 좌표
> 조회에 같은 파일을 `state.source_url` 로 재사용한다(별도 corpus env 없음 — 좌표계 어긋남 방지).
