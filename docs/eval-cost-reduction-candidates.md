...
### 2. `reason` 필드 제거 — 총비용 **21.3% (실측)**, 단 컨텍스트 지표 +0.05 관대화
...
**2차 재검증 결과 (2026-08-11, KorQuAD 475청크 · dense 폴백 0 · clean 변형 · A1/A2/B1/B2 각 20 probe):**

| | A(유지) 평균 | B(제거) 평균 | 실측 |
|---|---|---|---|
| 출력 토큰/호출 | 1,634 | 1,096 | **−33.0%** |
| 입력 토큰/호출 | 6,338 | 5,943 | −6.2% |
| 판정 비용 | — | — | **−21.3%** |
| 파싱 실패 | 3/40 | **0/40** | max_tokens 절단 감소 |

품질 (자기일치 바닥선 대비):

- `faithfulness`·`correctness`: 차이가 바닥선 이내 — **효과 없음**
- `context_precision` +0.03~0.06, `context_recall` +0.05~0.06 — **일관된 상향(관대화)**.
  두 쌍(A1B1·A2B2) 모두 같은 방향, 1차 실험에서도 recall 만 바닥선을 벗어났던 것과 일치.
  reason 강제가 없으면 심판이 attributed=1 을 더 쉽게 준다.

**판단**: 절대 점수가 +0.05 이동하므로 과거 실행과의 비교·임계값(`RAGAS_*_MIN`)에 영향.
선택지 — (a) 전면 제거 + 임계값 재캘리브레이션, (b) 관대화가 확인된 두 블록
(`context_verdicts`/`recall_classifications`)만 reason 유지하고 나머지 제거(절감 ~12–15% 추정).
후보 5(rule-선별 게이트) 도입 시 두 블록이 gold 기반 rule 로 대체되므로 (b)는 자연 소멸.
