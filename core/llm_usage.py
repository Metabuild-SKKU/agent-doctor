...
_PRICES_USD_PER_1M = {
    ...
    "gpt-4.1": (2.00, 8.00),
    # 추론 모델(core/llm_clients.py 가 max_completion_tokens 로 호출). 내부 추론 토큰도
    # output 으로 과금되므로 여기 단가가 비면 가장 비싼 호출이 집계에서 빠진다.
    # 접두 매칭이라 -pro 변형은 별도 등록해야 본단가로 잘못 계산되지 않는다.
    "gpt-5-chat-latest": (5.00, 30.00),   # 추론 모델 아님(temperature 허용)
