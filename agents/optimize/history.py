...
#      σ(한 번 측정) 0.0084 · σ_Δ(두 측정의 차이) 0.0120 · |Δ| 95분위 0.0233
# (되뽑기 편향 보정 ×1.118 이 반영된 값이다 — 그 보정 전 도구가 낸 수치는 이보다 작다.)
# 마진은 σ 가 아니라 **σ_Δ** 로 정한다 — 판정은 서로 다른 두 config 를 각각 한 번씩
# 잰 차이에 걸리므로, 독립 두 측정의 차이 분포가 기준이다(σ_Δ ≈ √2·σ).
# max(2·σ_Δ, P95) = 0.0239 → 판정 눈금(0.01)으로 올려 0.03. 값을 묶는 쪽은 2·σ_Δ 다.
# 옛 값 0.02 는 |Δ| 95분위(0.0233)보다 작다 — 같은 config 를 두 번 잰 차이가 그만큼
# 벌어지는 일이 20번에 한 번 넘게 있었고, 그중 위로 벌어진 절반이 그대로 가짜 KEEP
# 이다(40번에 한 번 이상). sweep 은 후보 T개 중 최고를 baseline 과 비교해(승자의 저주)
# 더 큰 마진이 필요한데(후보 2~4개 → 0.0227~0.0256), 0.03 이면 그쪽도 함께 덮는다.
#
