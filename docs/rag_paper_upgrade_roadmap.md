...
| 7 | 처방 선택을 밴딧으로 | AutoRAG-HP | 🟡 **7-a(티어 가중치화)는 지금 가능** · 7-b(밴딧)는 🔒 2번 |
| 8 | 라벨 정확도 측정 | Doctor-RAG | 🟡 **8-a·b 는 지금 가능** · 8-c·d 는 🔒 코퍼스 배관 |

**2번에 걸린 것과 아닌 것을 구분해 둔다.** 6·7-b 는 종합점수 노이즈(점수 축)에 걸리지만,
7-a 는 설계 판단이고 8 은 진단 정확도(다른 축)라 σ 와 무관하다. 통째로 🔒 로 두면
"실측 전에는 아무것도 못 한다"로 읽히는데 사실이 아니다.
...
#### 두 조각이고, 앞 하나는 측정 없이 판단할 수 있다

| | 조각 | 2번 필요? |
|---|---|---|
| **7-a** | 티어를 하드 1순위에서 **가중치**로 | ❌ 설계 판단이지 측정 문제가 아니다 |
| 7-b | 밴딧 도입(실적으로 기대값 갱신) | ⭕ 예산 안에 수렴하는지 알아야 한다 |

`action_aggregator.py` 의 docstring 이 티어 우선을 *"불변조건. 점수보다 먼저다"* 로 명시한다 —
버그가 아니라 의도된 설계다. 따라서 7-a 는 **그 불변조건을 유지할지**를 정하는 일이고,
σ 를 몰라도 논의할 수 있다.

7-b 가 2번에 걸리는 이유는 문서가 처음 적은 "보상이 노이즈면 학습 자체가 안 된다" 보다
정확히는 **예산**이다. 밴딧은 원래 불확실성 하에서 탐색하라고 만든 도구라 노이즈 자체가
결격 사유는 아니다. 문제는 `max_optimize_visits`(20) 안에 팔을 구분할 수 있느냐이고,
노이즈가 효과 크기만 하면 수렴하지 못한다. 2번이 답하는 게 그 비율이다.

> 다만 7-a 의 **근거**도 완전히 깨끗하진 않다. 위 표의 "롤백/턱걸이" 판정 자체가 노이즈 낀
> 마진으로 내려진 것이다. 구조적 사실(점수 4.0 이 1.0 을 이길 수 없다)은 측정과 무관하게
> 참이지만, **그게 나쁘다는 증거**는 마진에 의존한다.
...
### 8. 라벨 정확도 측정 — 네 조각, 앞 둘은 지금 가능

Doctor-RAG(arXiv 2604.00865)가 라벨별 정확도 표를 싣는다(Format 94.9% ~ Search 62.0%).
우리는 **진단이 맞는지**를 체계적으로 재본 적이 없다.

**2번(σ 실측)과 무관하다.** 2·6·7 은 "종합점수가 얼마나 흔들리나"(점수 축)이고 8 은
"우리 라벨이 맞나"(진단 정확도 축)다. 8 을 통째로 🔒 로 두면 "아무것도 못 한다"로 읽히는데,
실제로 막힌 건 뒤 두 조각뿐이다.

| | 조각 | 막는 것 | 비용 |
|---|---|---|---|
| **8-a** | 격자 케이스 확충 (`tests/diagnose_grid/`) | **없음** | ~0 |
| **8-b** | RAGEC 16개 ↔ 우리 31개 대조표 | **없음**(CSV 만 있으면 됨) | 0 |
| 8-c | DragonBall 원본 코퍼스 → 파이프라인 배관 | 🔒 큰 작업 | — |
| 8-d | 실제 라벨 정확도 측정 | 8-c | API |

#### 8-a. 격자 케이스 확충 — 지금 가능

#106 이 틀을 넣었다(`tests/diagnose_grid/`). 사람이 **상황을 먼저 쓰고 기대 라벨을 정하면**
진단 파이프라인을 실제로 태워 대조한다.
...
`LABELS.md` 가 **구현(임계값·판별 신호·슬롯 우선순위)을 일부러 뺐다** — 작성자가 그걸 보고
기대 라벨을 정하면 코드가 자기 자신을 채점하는 꼴이 되기 때문이다. 순서는 상황 → 무엇을
고쳐야 하나 → 그 처방을 가진 라벨이다.

`known_gap` 은 **실패를 기대하는 케이스**다. 진단이 고쳐져 통과하기 시작하면 테스트가
알려준다. 이미 하나 잡혀 있다.

> `chunking_overchunking` 기대 → 현재 진단은 `retrieval_low_rank` 를 낸다. A 슬롯에서 청킹
> 라벨은 `qtype=bridge` 일 때만 채택되고, 그 외에는 순위·코퍼스 라벨이 선점한다.

**케이스 7개 / 라벨 31개.** 대부분의 라벨은 "이 상황이면 이 라벨" 이 한 번도 확인된 적이
없다. 늘리는 데 API 비용도(`compute_ragas=False` 면 LLM 미사용) 외부 데이터도 필요 없다.

한계도 분명하다 — 사람이 만든 상황이라 **"실제로 그런 실패가 일어나는가"** 는 답하지 못한다.
격자가 답하는 것은 **"그 상황이 오면 제대로 잡는가"** 하나다. 전자는 8-c·d 몫이다.

#### 8-b. 대조표 — 지금 가능

정답지: [layer6ai-labs/rag-error-classification](https://github.com/layer6ai-labs/rag-error-classification)
(MIT) — 사람이 라벨링한 RAG 에러 **377건**.

코퍼스 없이 CSV 만으로 **그들의 16개가 우리 31개 중 어디에 대응하는지** 표를 만들 수 있다.
그러면 8-c 가 끝나는 즉시 채점할 수 있고, 그 전에도 **대응이 없는 라벨 = 택소노미 공백**이
드러난다. E15 판정 재료도 여기서 나온다(시간 순서 59건이 우리 어느 라벨로 분류되는지).
...
**정답지를 얻는 것이자 질문 종류를 얻는 것이다.** 아래 분포표대로 우리 평가셋은 100% 단순
사실이라 31개 중 19개가 원리적으로 미발화한다. 코퍼스 교체는 8 의 전제이면서 동시에
**3번(E15) 판정의 전제**이기도 하다(시간 순서 59건).

#### 그들의 택소노미로 갈아타지는 않는다

RAGEC 16개와 우리 31개는 **만든 목적이 다르다.**

| | RAGEC | 우리 |
|---|---|---|
| 목적 | 분류(무슨 종류의 실패인가) | **처방**(무엇을 고쳐야 하나) |
| 라벨 기준 | 현상이 다르면 다른 라벨 | **처방이 다르면** 다른 라벨 |
| 소비처 | 사람이 읽음 | `rules.py` → 실제 config 패치 |

분류용 택소노미로 갈아타면 처방으로 이어지지 않는 라벨이 생겨 라벨→처방→패치 사슬이
끊긴다. E14 를 접은 근거(현상은 다른데 처방이 같다)도 이 원칙에서 나온다. **정답지로 쓰되
택소노미는 우리 것을 유지한다.**
