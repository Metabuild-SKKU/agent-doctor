...
[Phase 2 범위]  판정·롤백·블랙리스트·pending 이력까지.
  사용자 리포트 저장(state 정식 필드)은 팀 합의 대기라 아직 미연결.
  '마지막 처방 미판정' 한계는 graph 라우팅 보강으로 해소 예정(별도 단계).
"""
from __future__ import annotations

from core.state import AgentDoctorState
from agents.optimize import planner, optimizer, config_mapper, history
...
def run(state: AgentDoctorState) -> AgentDoctorState:
    ...
    try:
        ...
        rolled_back = _judge_pending_trial(state)
...
def _judge_pending_trial(state: AgentDoctorState) -> bool:
    """직전에 적용한 처방(pending)을 판정한다. 나빴으면 config 롤백 + blacklist.
    롤백을 수행했으면 True 를 반환한다(재색인 필요 신호)."""
