...
왜 필요한가:
    ...
    진행률 쪽은 '완료 이벤트 기반' 이라는 계약이 핵심이다 — 시간이 지나도 tick 이
    없으면 안 찍힌다. 문서만 그렇게 적어 두면 나중에 타이머로 바꿔 놓고도 테스트가
    통과할 수 있어서, 그 성질 자체를 테스트로 세운다.
"""
...
class ParallelMapContractTest(unittest.TestCase):
    ...
    def test_여러_실패시_먼저_끝난_예외가_올라온다(self):
        """pool.map 대비 실제 동작 차이 — '입력 순서상 첫 예외'가 아니다.

        docstring 에만 적어 두면 나중에 누가 '입력 순서 기준 실패'로 기대하고 쓴다.
        어느 쪽이 올라오는지를 실행으로 고정해 둔다."""
        class _Slow(RuntimeError):
            pass

        class _Fast(RuntimeError):
            pass

        def fail_at_different_speeds(x: int):
            if x == 0:
                time.sleep(0.3)      # 입력 순서로는 첫 번째지만 가장 늦게 끝난다
                raise _Slow("느린 실패")
            if x == 5:
                raise _Fast("빠른 실패")
            time.sleep(0.3)
        ...
        with self.assertRaises(_Fast):
            parallel_map(fail_at_different_speeds, list(range(10)), max_workers=8)
...
class ParallelMapProgressTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # 아주 작은 양수로 만든다.
        self._env = {k: os.environ.get(k) for k in ("PROGRESS_LOG", "PROGRESS_MIN_INTERVAL_SEC")}
    ...
    def test_라벨을_안_주면_아무것도_찍지_않는다(self):
        os.environ["PROGRESS_MIN_INTERVAL_SEC"] = "0.001"
        lines: list[str] = []
        with mock.patch("builtins.print", side_effect=lambda *a, **k: lines.append(" ".join(map(str, a)))):
    ...
    def test_주기가_지나면_진행률과_완료줄을_찍는다(self):
        lines = self._run(PROGRESS_MIN_INTERVAL_SEC="0.001", PROGRESS_LOG="1")
    ...
    def test_주기가_안_지나면_완전히_침묵한다(self):
        ...
        self.assertEqual(self._run(PROGRESS_MIN_INTERVAL_SEC="3600", PROGRESS_LOG="1"), [])

    def test_PROGRESS_LOG_0_이면_꺼진다(self):
        self.assertEqual(self._run(PROGRESS_MIN_INTERVAL_SEC="0.001", PROGRESS_LOG="0"), [])

    def test_off_와_false_도_끄는_값이다(self):
        for value in ("off", "false", "FALSE", "Off"):
            with self.subTest(value=value):
                self.assertEqual(self._run(PROGRESS_MIN_INTERVAL_SEC="0.001", PROGRESS_LOG=value), [])

    def test_병렬_경로에서도_진행률이_나오고_순서는_유지된다(self):
        os.environ.update(PROGRESS_LOG="1", PROGRESS_MIN_INTERVAL_SEC="0.001")
        lines: list[str] = []
        with mock.patch("builtins.print", side_effect=lambda *a, **k: lines.append(" ".join(map(str, a)))):
...
class ProgressCompletionDrivenTest(unittest.TestCase):
    """'완료 이벤트 기반' 계약 — 시간이 아니라 tick 이 출력을 만든다.

    리뷰 지적(#1): 문서가 'PROGRESS_MIN_INTERVAL_SEC 마다 찍힌다'로 읽히면 사용자가
    기대하는 보장과 구현이 어긋난다. 실제 계약은 '항목이 끝났을 때, 마지막 출력
    이후 최소 간격이 지났으면 찍는다' 이고, 그 성질을 여기서 세운다."""

    def setUp(self):
        patcher = mock.patch.dict(os.environ, {"PROGRESS_LOG": "1",
                                               "PROGRESS_MIN_INTERVAL_SEC": "0.01"})
        patcher.start()
        self.addCleanup(patcher.stop)

    def test_긴_단건이_도는_동안은_간격이_지나도_침묵한다(self):
        """알려진 한계를 문서가 아니라 실행으로 고정한다.

        이게 싫으면 별도 타이머 스레드가 필요하다 — 그때 이 테스트가 '계약이
        바뀐다'는 신호로 같이 깨져야 한다."""
        lines: list[str] = []

        def one_long_call(x):
            time.sleep(0.3)          # 최소 간격(0.01초)의 30배
            return x

        with mock.patch("builtins.print",
                        side_effect=lambda *a, **k: lines.append(" ".join(map(str, a)))):
            parallel_map(one_long_call, [1], max_workers=1, label="[테스트] 단건")

        self.assertEqual(lines, [], "항목이 하나면 완료 전까지 찍을 게 없다")

    def test_tick_이_없으면_시간이_지나도_안_찍는다(self):
        """리포터를 직접 만들어 두고 기다리기만 한다 — 타이머라면 여기서 줄이 나온다."""
        lines: list[str] = []
        with mock.patch("builtins.print",
                        side_effect=lambda *a, **k: lines.append(" ".join(map(str, a)))):
            reporter = progress.start("[테스트] 대기", 100)
            time.sleep(0.05)         # 최소 간격의 5배
            progress.finish(reporter)
        self.assertEqual(lines, [])

    def test_tick_이_있으면_그제서야_찍는다(self):
        lines: list[str] = []
        with mock.patch("builtins.print",
                        side_effect=lambda *a, **k: lines.append(" ".join(map(str, a)))):
            reporter = progress.start("[테스트] 대기", 100)
            time.sleep(0.05)
            progress.tick(reporter)  # 이 호출이 출력을 만든다
            progress.finish(reporter)
        self.assertEqual(len(lines), 2, lines)      # 진행줄 + 완료줄
        self.assertIn("1/100", lines[0])


class ProgressAbortTest(unittest.TestCase):
    """실패 종료가 정상 완료와 구분되는지 (리뷰 지적 #2).

    진행 줄이 '3/30' 에서 그냥 끊기면 로그만 보고 '도는 중'과 '예외로 죽음'을
    구분할 수 없다. 관측성이 목적인 기능에서 그 구분이 안 되는 건 결함이다."""
    ...
    def _capture(self, fn, workers):
        lines: list[str] = []
        with mock.patch("builtins.print",
                        side_effect=lambda *a, **k: lines.append(" ".join(map(str, a)))):
            with self.assertRaises(_Boom):
                parallel_map(fn, list(range(20)), max_workers=workers, label="[테스트] 실패")
        return lines
    ...
    def test_순차_경로에서_중단줄로_닫힌다(self):
        lines = self._capture(self._boom_late, 1)
        self.assertTrue(lines, "진행줄이 몇 줄 나온 뒤 실패하는 시나리오여야 한다")
        self.assertIn("중단", lines[-1])
        self.assertNotIn("완료", lines[-1])

    def test_병렬_경로에서도_중단줄로_닫힌다(self):
        lines = self._capture(self._boom_late, 4)
        self.assertIn("중단", lines[-1])

    def test_침묵했던_단계는_중단줄도_안_찍는다(self):
        """예외는 어차피 전파돼 호출부가 보고한다. 조용했던 단계에 한 줄만 얹지 않는다."""
        lines: list[str] = []
        with mock.patch.dict(os.environ, {"PROGRESS_MIN_INTERVAL_SEC": "3600"}), \
             mock.patch("builtins.print",
                        side_effect=lambda *a, **k: lines.append(" ".join(map(str, a)))):
            with self.assertRaises(_Boom):
                parallel_map(self._boom_late, list(range(20)), max_workers=1,
                             label="[테스트] 조용한 실패")
        self.assertEqual(lines, [])

    def test_완료줄과_중단줄이_같이_나오지_않는다(self):
        lines = self._capture(self._boom_late, 1)
        self.assertEqual(sum("완료" in line for line in lines), 0)
        self.assertEqual(sum("중단" in line for line in lines), 1)

    def test_중단줄도_cp949_안전(self):
        for line in self._capture(self._boom_late, 1):
            line.encode("cp949")
...
class ProgressFormatTest(unittest.TestCase):
    ...
    def test_잘못된_주기는_기본값으로_흘린다(self):
        ...
        for bad in ("", "abc", "0", "-5"):
            with mock.patch.dict(os.environ, {"PROGRESS_MIN_INTERVAL_SEC": bad}):
                self.assertEqual(progress._min_interval_sec(), progress._DEFAULT_MIN_INTERVAL_SEC)

    def test_유효한_주기는_그대로_읽는다(self):
        with mock.patch.dict(os.environ, {"PROGRESS_MIN_INTERVAL_SEC": "2.5"}):
            self.assertEqual(progress._min_interval_sec(), 2.5)
