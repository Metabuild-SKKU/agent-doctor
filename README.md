...
실행 로그에는 **사용자가 실제로 던진 질문, 검색된 문서 원문, 답변, 정답지**가 그대로 들어갑니다. 사내 문서나 고객 문의 기록이라면 개인정보·기밀이 섞여 있을 가능성이 높습니다. 아래를 확인하고 넘겨주세요.

- **진단서에 원문이 실립니다.** `report.html`의 "실패한 검증 질문" 섹션은 질문·정답·실제 답변을 그대로 보여줍니다. 진단서를 그대로 공유하면 로그 내용도 함께 나갑니다.
- **LLM 심판에 전송됩니다.** RAGAS 채점은 질문·컨텍스트·답변을 심판 provider(OpenAI/Anthropic/Gemini/OpenRouter)로 보냅니다. 외부 전송이 곤란하면 `--no-llm`으로 규칙 지표만 쓰거나, 로컬 심판을 쓸 수 있는 환경에서 돌리세요(대신 생성축 라벨 4종이 침묵합니다).
- **업로드 파일이 서버에 남습니다.** 웹 UI로 올린 로그·골든셋은 `uploads/<run_id>/`에 저장되고 자동 삭제되지 않습니다(`.gitignore` 대상이라 커밋되지는 않습니다). 진단이 끝나면 직접 지우세요.
- **공유 전 익명화·샘플링을 권합니다.** 진단에는 50~150건이면 충분하므로(권장 크기), 전체 로그 대신 대표 표본만 익명화해 넘기는 쪽이 안전합니다.
