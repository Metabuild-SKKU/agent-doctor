...
| | 무엇과 비교 | 커버리지 | 비용 |
|---|---|---|---|
| **A. RAGEC 대조** | 공개 데이터셋의 사람 라벨 377건 | 32개 중 14개 | 실행 1회 |
...
```
A →  "RAGEC 사람 라벨과의 일치도 X% (14/32 라벨, 호환 표본에 한함)"
B →  "진단 유효성 X%"
```
...
**① RAGEC로는 32개 중 14개만 검증됩니다.** 특히 C그룹(컨텍스트 구성) 3개는 정답지에 대응 사례가 **0건**이라 전혀 검증되지 않습니다. E11·E15도 0건입니다. 자체 라벨링(B)이 이 구멍을 메웁니다.
