...
### KorQuAD 2.1 데이터셋으로 평가

사람이 만든 골든 QA(=`taxonomy` 소스)로 진단하려면 `data/corpus.jsonl`(이미 청킹된
코퍼스) + `data/qa_pairs.jsonl`(정답+`positive_chunk_ids`)을 두고 전용 드라이버를 쓴다.
...
KORQUAD_EMBED=0 python run_korquad_eval.py                  # 임베딩 스킵 → keyword(BM25)
EVAL_MODE=deep EVAL_ENABLE_LLM=1 python run_korquad_eval.py # RAGAS·생성 채점(LLM 키·비용)
```

로더(`agents/eval/datasets/korquad.py`)가 `corpus.chunk_id`→`Chunk`,
`qa.positive_chunk_ids`→`Probe.gold_chunk_ids` 로 직접 매핑하고, 드라이버가 그 probe 를
`eval_probes.json` 에 써서 `EVAL_PROBE_SOURCE=made` 로 주입한다 → **재청킹·resync 없이
gold 가 정확히 맞는다**. 코퍼스가 이미 청킹돼 있어 Index 청킹을 건너뛰므로 chunk_size
계열 처방은 이 데이터셋엔 적용되지 않는다(검색·생성 진단은 그대로 유효).
