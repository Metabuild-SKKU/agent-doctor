...
- ~~**마진 값 자체의 재보정 (최우선 후속)**: `0.02`는 **잠정치이며 노이즈 분포를
  ...
  마진을 `k·σ`로 다시 정해야 한다.~~ → ✅ 완료. #142가 측정 하네스
  (`tools/measure_eval_sigma.py`)를, #143이 재보정을 넣었다. 같은 config 5회 반복
  측정으로 σ_Δ=0.0120을 얻어 `max(2·σ_Δ, P95)=0.0239`를 판정 눈금(0.01)으로 올려
  **`0.03`** 이 됐다. 마진은 `k·σ`가 아니라 **`k·σ_Δ`** 로 정한다 — 판정은 서로 다른
  두 config를 각각 한 번씩 잰 차이에 걸리기 때문이다. 근거는
  `agents/optimize/history.py`의 `MIN_IMPROVEMENT_MARGIN` 주석에 있다.
  조건(코퍼스·QA셋 크기·judge 모델)이 바뀌면 다시 재야 하고, 그 신호는
  `max_repeated_measurement_spread` 경고가 준다.
