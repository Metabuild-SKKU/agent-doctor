...
| 5 | 흔들리는 심판 신호가 라벨을 뒤집지 못하게 | 심판 감사 | ⬜ 미착수 |
...
| | 방법 | 평가 |
|---|---|---|
| **A** | 조건 미달 시 라벨을 **없애는** 대신 `confirmed=False`(예비)로 **강등** | 라벨이 사라지지 않고 가중치만 절반. 최소 변경 |
| **B** | 이 조건을 **글자 기반**으로 교체 — 답 문자열이 검색된 청크 안에 있나 | 결정론적·비용 0. 추출형 단답엔 잘 맞고 서술형엔 약함 |
| **C** | 심판을 여러 번 불러 중앙값 | **불가** — 아래 참고 |

C 가 막힌 이유: 심판 감사 논문(arXiv 2607.08535)이 **"같은 모델을 여러 번 돌린 배심원단은
에러 상관 ρ=0.944~0.972"** 라고 보고한다. 같은 모델은 같은 곳에서 같이 틀리므로 평균이
분산을 줄이지 못한다.

현재 grounding 판정은 전부 `faithfulness` 기반이다(`_grounded_ok`·`_grounded_verified`·
`_retrieval_verified_grounded`). B 를 택하면 결정론적 대안을 새로 만들어야 한다.
