...
def _split_markdown_sections(text: str) -> list[_ChunkDraft]:
    """Markdown 제목 경계를 먼저 찾아 section 정보를 보존한다."""
    heading_pattern = re.compile(r"^(#{1,6})\s+(.+?)\s*$")
    heading_path: list[str] = []
    section_lines: list[str] = []
    sections: list[_ChunkDraft] = []
    current_section: str | None = None

    def flush() -> None:
        body = "\n".join(section_lines).strip()
        if body:
            sections.append(_ChunkDraft(text=body, section=current_section))
    ...
    flush()
    return sections or [_ChunkDraft(text=text)]
...
def _recursive_chunks(text: str, chunk_size: int, chunk_overlap: int) -> list[str]:
    """문맥 경계를 우선하면서 최대 크기와 overlap을 지키는 분할기."""
    text = text.strip()
    if not text:
        return []
...
def _chunk_text(text: str, chunk_size: int, chunk_overlap: int) -> list[str]:
    """기존 테스트와 호출부를 위한 텍스트 청킹 공개 함수."""
    return _recursive_chunks(_normalize_text(text), chunk_size, chunk_overlap)
...
def _chunk_document(document: Document, chunk_size: int, chunk_overlap: int) -> list[_ChunkDraft]:
    ...
    for section in _split_markdown_sections(normalized):
        for text in _recursive_chunks(section.text, chunk_size, chunk_overlap):
            drafts.append(_ChunkDraft(text=text, section=section.section))
    return drafts
