...
- reranker OFF/ON을 별도 scenario로 실행하고 전체 trial budget을 나눠 사용한다.
...
[확인된 문제: strict hybrid 보장 실패]
- retriever.search_type=["hybrid"]를 전달하면 현재 adapter는 vector similarity와
  BM25를 RAGBuilder retriever 후보 두 개로 변환한다.
- RAGBuilder 0.1.6은 후보 두 개를 반드시 ensemble로 묶지 않고 trial마다 하나만
  선택하거나 둘을 조합할 수 있다. 따라서 hybrid만 요청해도 dense 또는 BM25
  단독 결과가 반환될 수 있다.
- 현재 결과 정규화는 반환된 retriever type이 요청 search space 안에 있는지까지
  검증하지 않아, 이 단독 결과가 completed trial로 남을 수 있다.
- 후속 구현에서는 결과에 outside_search_space 검증을 추가하고, dense/BM25/hybrid
  scenario를 분리해야 한다. strict hybrid는 vector+BM25 EnsembleRetriever를 만드는
  custom retriever 하나로 RAGBuilder에 전달해 내부에서 분해되지 않도록 할 계획이다.
...
class RAGBuilderAdapter:
    ...
    def _extract_best_score(
        ...
        """0점을 포함해 외부 best score를 보존하고 없을 때만 trial에서 보완한다."""
